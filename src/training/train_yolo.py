...
def main():
    ...
    results = model.train(
        ...
        # --- LEARNING RATE (lowered for fine-tuning) ---
        lr0=0.001,  # 10x lower than default — prevents overshooting on small dataset
        lrf=0.01,  # Final LR = lr0 * lrf = 0.00001
        cos_lr=True,  # Cosine annealing — smoother convergence to better minima
        warmup_epochs=5,  # Longer warmup for stability on small dataset
        # --- AUGMENTATION STRATEGY (ENABLED — previously commented out) ---
        mosaic=1.0,  # Combines 4 floor plans into one (critical for small datasets)
        close_mosaic=20,  # Turn off mosaic for last 20 epochs for fine-tuning
